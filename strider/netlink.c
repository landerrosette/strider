...
static int strider_nl_add_pattern_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (*set_name == '\0')
		return -EINVAL;
	return strider_set_add_pattern(genl_info_net(info), set_name,
				       nla_data(info->attrs[STRIDER_ATTR_PATTERN]),
...
static int strider_nl_del_pattern_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (*set_name == '\0')
		return -EINVAL;
	return strider_set_del_pattern(genl_info_net(info), set_name,
				       nla_data(info->attrs[STRIDER_ATTR_PATTERN]),
				       nla_len(info->attrs[STRIDER_ATTR_PATTERN]));
}
