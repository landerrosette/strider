...
static struct strider_ac_node *strider_ac_node_find_next(const struct strider_ac_node *node,
							 u8 byte)
{
...
static void strider_ac_trie_destroy(struct strider_ac_trie *trie)
{
	...
	while (!list_empty(&queue)) {
		struct strider_ac_node *node =
			list_first_entry(&queue, struct strider_ac_node, list);
		list_del(&node->list);
		{
				       ...
				       void *iter_ctx)
{
	for (const struct strider_ac_target *target; (target = get_target(iter_ctx));) {
		...
		for (size_t i = 0; i < target->pattern_len; ++i) {
			struct strider_ac_node *child =
				strider_ac_node_find_next(node, target->pattern[i]);
			if (!child) {
	...
	while (!list_empty(&queue)) {
		...
		list_for_each_entry(tsn, &node->transitions, list) {
			...
			for (const struct strider_ac_node *f = node->failure;; f = f->failure) {
				struct strider_ac_node *fnext =
					strider_ac_node_find_next(f, tsn->byte);
				if (fnext) {
					child->failure = fnext;
					break;
				}
...
static int strider_ac_transition_cmp(void *priv, const struct list_head *a,
				     const struct list_head *b)
{
	const struct strider_ac_transition *tsn_a =
		list_entry(a, struct strider_ac_transition, list);
	const struct strider_ac_transition *tsn_b =
		list_entry(b, struct strider_ac_transition, list);
...
static int strider_ac_trie_assign_state_ids(struct strider_ac_trie *trie)
{
	...
	while (!list_empty(&stack)) {
		...
		u8 first_byte =
			list_first_entry(&node->transitions, struct strider_ac_transition, list)
				->byte;
		u8 last_byte =
			list_last_entry(&node->transitions, struct strider_ac_transition, list)
				->byte;
