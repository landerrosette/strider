...
static int strider_ac_test_match_cb(const struct strider_ac_target *ac_target, size_t pos,
				    void *ctx)
{
	struct strider_ac_test_match_info *info = ctx;
	++info->match_count;
	unsigned int found_id =
		container_of(ac_target, struct strider_ac_test_target, ac_target)->id;
...
{
	strider_ac_test_case_run(test, (const char *[]){ "he", "she", "his", "hers", NULL },
				 "ahishers", 4, 1U << 0 | 1U << 1 | 1U << 2 | 1U << 3);
}

static void test_multi_overlap_sequences(struct kunit *test)
{
	strider_ac_test_case_run(test, (const char *[]){ "ana", "nana", "banana", NULL },
				 "bananana", 6, 1U << 0 | 1U << 1 | 1U << 2);
}

static void test_repeated_char_overlaps(struct kunit *test)
{
	strider_ac_test_case_run(test, (const char *[]){ "a", "aa", "aaa", NULL }, "aaaaa", 12,
				 1U << 0 | 1U << 1 | 1U << 2);
}
...
static void test_shared_prefix_varied_lengths(struct kunit *test)
{
	strider_ac_test_case_run(
		test, (const char *[]){ "a", "ab", "bab", "bc", "bca", "c", "caa", NULL }, "abccab",
		7, 1U << 0 | 1U << 1 | 1U << 3 | 1U << 5);
}

static void test_nested_prefix_suffix_outputs(struct kunit *test)
{
	strider_ac_test_case_run(test, (const char *[]){ "abc", "abcd", "bc", NULL }, "zabcabcd", 5,
				 1U << 0 | 1U << 1 | 1U << 2);
}

static void test_no_match(struct kunit *test)
{
	strider_ac_test_case_run(test, (const char *[]){ "hello", "world", NULL }, "well, hi there",
				 0, 0);
}
